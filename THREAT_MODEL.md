...
**Test coverage**: See `src/__tests__/security.test.ts` — traversal, symlink escape, boundary (`/foo` vs `/foobar`). See `src/__tests__/security-fuzz.test.ts` for adversarial fixture trees — long symlink chains, escapes mid-chain, detours that land back inside, symlink cycles, `..` applied to symlinked parents (swept over every 4-segment payload combination), case-insensitive filesystem edges, and NFC/NFD normalization collisions. See `src/__tests__/security-regression.test.ts` for the two bypasses that sweep found. See `src/__tests__/security-fuzz-depth.test.ts` for length-boundary and re-check classes — deeply nested trees (300 levels), symlink escapes planted at depth, lexical paths past `PATH_MAX`, components at and past `NAME_MAX` (`ENAMETOOLONG` must not read as "missing, therefore inside"), and deep `..` chains climbing above the root.

**Resolver statelessness (TOCTOU-adjacent)**: `SecurityPolicy` keeps no per-path state, so every call re-reads the tree and no verdict outlives the filesystem that justified it. `security-fuzz-depth.test.ts` pins this with sequential swaps — a link repointed outside after being accepted is rejected on re-check, one repointed inside after being rejected is accepted, a pending path that becomes an escaping symlink is rejected, and a directory replaced by an escaping symlink is rejected — with the sync and async resolvers agreeing across the swap. This guards against a cache being added later without noticing the consequence.

**Not covered**: a swap racing a single `realpath` call. The check and the caller's subsequent read are separate syscalls, so a sufficiently privileged local attacker who can write inside a configured project root can in principle swap a path between them. That is not deterministically testable without a seam in the resolver, and no test here should be read as a claim of resistance to it. The exposure is bounded by the same trust assumption as the rest of this model: an attacker who can already write arbitrary symlinks inside an allowlisted root.
